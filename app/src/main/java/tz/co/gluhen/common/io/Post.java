...
public class Post implements Closeable {
    ...
    private Post(ProgressObserver observer){this.io=new IO(observer);}
    public static Header requestHeaders(ProgressObserver observer){ return new Header(new Post(observer));}
    public  Post post(String url,int contentLength){
          try {
    ...
    public Post headers(Consumer<Map<String,String>> headerConsumer){
    ...
    public Post response(BiConsumer<IO,BufferedInputStream> streamConsumer){
    ...
    public Post response(Consumer<BufferedInputStream> streamConsumer){
        if(connection==null){return this;}
        try(BufferedInputStream in=new BufferedInputStream(connection.getInputStream())){
            headers.setRespHeaders(connection);
    ...
    public Post writeBody(byte[] b) { io.copy(new ByteArrayInputStream(b),outputStream);return this;}
    public Post writeBody(String b) { io.text(b,outputStream);return this;}
    public Post writeBody(Consumer<BufferedOutputStream> outputStreamConsumer) {
    ...
    public static  class Header {
        ...
        public Post openPost(String url,int contentLength) {p.headers=this;return p.post(url,contentLength);}

        public Post sendPost(String url,String body){return sendPost(url,body.getBytes());}
        public Post sendPost(String url,byte[] body){
            p.headers=this;
            return p.post(url,body.length).writeBody(o->p.io.copy(new ByteArrayInputStream(body),o));}

        public Post sendGet(String url) {p.headers=this;return p.get(url);}
        public Post openGet(String url) {p.headers=this;return p.get(url);}
